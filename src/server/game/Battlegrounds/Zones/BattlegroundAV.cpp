...
void BattlegroundAV::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    UpdateScore(player->GetTeam(true), -1);
}
...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0])
    {
        ...
        ChangeMineOwner(AV_NORTH_MINE, killer->GetTeam(true));
        ...
        ChangeMineOwner(AV_SOUTH_MINE, killer->GetTeam(true));
}
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    uint8 team = GetTeamIndexByTeamId(player->GetTeam(true));
    ...
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            if (m_Team_QuestStatus[team][0] == 500 || m_Team_QuestStatus[team][0] == 1000 || m_Team_QuestStatus[team][0] == 1500) //25, 50, 75 turn ins
            {
                ...
                for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT; ++i)
                    if (m_Nodes[i].Owner == player->GetTeam(true) && m_Nodes[i].State == POINT_CONTROLED)
                    {
                        DePopulateNode(i);
                        PopulateNode(i);
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            RewardReputationToTeam(team, 1, player->GetTeam(true));
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    if (owner == player->GetTeam(true) || m_Nodes[node].State != POINT_ASSAULTED)
        return;
...
WorldSafeLocsEntry const* BattlegroundAV::GetClosestGraveYard(Player* player)
{
    ...
    pGraveyard = sWorldSafeLocsStore.LookupEntry(BG_AV_GraveyardIds[GetTeamIndexByTeamId(player->GetTeam(true))+7]);
