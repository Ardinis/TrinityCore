        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[i].size())
            {
                ...
                else
                {
                    ...
                    m_CurrentPointPlayersCount[2 * i + GetTeamIndexByTeamId(player->GetTeam(true))]++;
                    ++j;
                }
            }
        }
        ...
        for (uint8 i = 0; i < m_PlayersNearPoint[point].size(); ++i)
        {
            ...
            if (player)
            {
                ...
                if (pointOwnerTeamId != m_PointOwnedByTeam[point])
                {
                    ...
                    if (m_PointState[point] == EY_POINT_STATE_UNCONTROLLED && player->GetTeam(true) == pointOwnerTeamId)
                        this->EventTeamCapturedPoint(player, point);

                    //point was under control and player isn't from team which controlled it
                    if (m_PointState[point] == EY_POINT_UNDER_CONTROL && player->GetTeam(true) != m_PointOwnedByTeam[point])
                        this->EventTeamLostPoint(player, point);
                }
                // hack fix for Fel Reaver Ruins
                if (point == FEL_REAVER && m_PointOwnedByTeam[point] == player->GetTeam(true))
...
void BattlegroundEY::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        case TR_BLOOD_ELF_POINT:
            if (m_PointState[BLOOD_ELF] == EY_POINT_UNDER_CONTROL && m_PointOwnedByTeam[BLOOD_ELF] == Source->GetTeam(true))
        ...
        case TR_FEL_REAVER_POINT:
            if (m_PointState[FEL_REAVER] == EY_POINT_UNDER_CONTROL && m_PointOwnedByTeam[FEL_REAVER] == Source->GetTeam(true))
                if (m_FlagState && GetFlagPickerGUID() == Source->GetGUID())
                    EventPlayerCapturedFlag(Source, BG_EY_OBJECT_FLAG_FEL_REAVER);
            break;
        case TR_MAGE_TOWER_POINT:
            if (m_PointState[MAGE_TOWER] == EY_POINT_UNDER_CONTROL && m_PointOwnedByTeam[MAGE_TOWER] == Source->GetTeam(true))
                if (m_FlagState && GetFlagPickerGUID() == Source->GetGUID())
                    EventPlayerCapturedFlag(Source, BG_EY_OBJECT_FLAG_MAGE_TOWER);
            break;
        case TR_DRAENEI_RUINS_POINT:
            if (m_PointState[DRAENEI_RUINS] == EY_POINT_UNDER_CONTROL && m_PointOwnedByTeam[DRAENEI_RUINS] == Source->GetTeam(true))
                if (m_FlagState && GetFlagPickerGUID() == Source->GetGUID())
...
void BattlegroundEY::EventPlayerDroppedFlag(Player* Source)
{
    ...
    if (Source->GetTeam(true) == ALLIANCE)
...
void BattlegroundEY::EventTeamCapturedPoint(Player* Source, uint32 Point)
{
    ...
    uint32 Team = Source->GetTeam(true);
...
void BattlegroundEY::EventPlayerCapturedFlag(Player* Source, uint32 BgObjectType)
{
    ...
    if (m_TeamPointsCount[team_id] > 0)
        AddPoints(Source->GetTeam(true), BG_EY_FlagPoints[m_TeamPointsCount[team_id] - 1]);
...
WorldSafeLocsEntry const* BattlegroundEY::GetClosestGraveYard(Player* player)
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        if (m_PointOwnedByTeam[i] == player->GetTeam(true) && m_PointState[i] == EY_POINT_UNDER_CONTROL)
        {
