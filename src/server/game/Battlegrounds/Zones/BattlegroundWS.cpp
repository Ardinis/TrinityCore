...
void BattlegroundWS::EventPlayerCapturedFlag(Player* Source)
{
    ...
    RewardHonorToTeam(GetBonusHonorFromKill(2), Source->GetTeam(true));
    ...
    UpdateFlagState(Source->GetTeam(true), 1);                  // flag state none
    UpdateTeamScore(Source->GetTeam(true));
    // only flag capture should be updated
    UpdatePlayerScore(Source, SCORE_FLAG_CAPTURES, 1);      // +1 flag captures

    // update last flag capture to be used if teamscore is equal
    SetLastFlagCapture(Source->GetTeam(true));
    ...
    {
        _flagsTimer[GetTeamIndexByTeamId(Source->GetTeam(true)) ? 0 : 1] = BG_WS_FLAG_RESPAWN_TIME;
    }
}
...
void BattlegroundWS::EventPlayerDroppedFlag(Player* Source)
{
    ...
    if (set)
    {
        ...
        UpdateFlagState(Source->GetTeam(true), 1);
        ...
        _flagsDropTimer[GetTeamIndexByTeamId(Source->GetTeam(true)) ? 0 : 1] = BG_WS_FLAG_DROP_TIME;
    }
}
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
    ...
    if (Source->GetTeam(true) == HORDE && this->GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_BASE
    ...
    if (Source->GetTeam(true) == ALLIANCE && this->GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_BASE
