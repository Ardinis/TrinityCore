...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    switch (petType)
    {
        case HUNTER_PET:
        {
            ...
            //        SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel * 4 - petlevel) + (GetTotalAttackPowerValue(BASE_ATTACK)*0.22 * 2 / 14)));
            //        SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel * 4 + petlevel) + (GetTotalAttackPowerValue(BASE_ATTACK)*0.22 * 2 / 14)));
			// les pets reçoivent 22% de la PA du chassou a voir si il se plaigne, test par gabi.
	        break;
        }
        default:
        {
            switch (GetEntry())
            {
                ...
                case 19668: // Shadowfiend
                {
                    ...
                    {
                        SetCreateMana(28 + 10 * petlevel);
                        SetCreateHealth(28 + 30 * petlevel + m_owner->SpellBaseDamageBonus(SPELL_SCHOOL_MASK_SHADOW) * 0.3);
                    }
               ...
               case 24207:
                   ...
                   SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, GetCreatureInfo()->mindmg);
                   SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, GetCreatureInfo()->maxdmg);
                   break;
            }
            break;
        }
...
void Pet::_LoadAuras(uint32 timediff)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                if (!aura->CanBeSaved() || (spellInfo->Id == 61669 && (!GetOwner() || (GetOwner() &&  !GetOwner()->HasAura(13161)))))
                {
                    aura->Remove();
                    continue;
                }
