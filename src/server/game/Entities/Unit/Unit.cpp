...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (cleanDamage && damagetype == DIRECT_DAMAGE && this != victim && getPowerType() == POWER_RAGE)
    {
        ...
        if (cleanDamage->hitOutCome == MELEE_HIT_DODGE || cleanDamage->hitOutCome == MELEE_HIT_BLOCK || cleanDamage->hitOutCome == MELEE_HIT_PARRY)
    ...
    if (!damage)
    {
        ...
        // Could aswell move the whole Cast / Channel - delay check before the !damage check but it would look more messy.
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    if (victim != this)
    {
        ...
        for (AuraEffectList::iterator itr = vSplitDamageFlatCopy.begin(); (itr != vSplitDamageFlatCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
        {
            ...
            uint32 healthLimit = 0;
            switch ((*itr)->GetSpellInfo()->Id)
            {
                // Divine Sacrifice
            case 64205:
                healthLimit = caster->CountPctFromMaxHealth(20);
                break;
            default:
                break;
            }
            ...
            if (caster->GetHealth() - splitDamage <= healthLimit)
            {
                caster->RemoveAura((*itr)->GetBase());
                continue;
            }
        ...
        for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(), next; (itr != vSplitDamagePctCopy.end()) &&  (dmgInfo.GetDamage() > 0); ++itr)
        {
            ...
            uint32 splitDamage1 = splitDamage;
            if (splitDamage > 0)
                splitDamage1 = splitDamage;
            else
                splitDamage1 = 0;

            (*itr)->GetBase()->CallScriptEffectSplitHandlers((*itr), aurApp, dmgInfo, splitDamage1);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    uint32 missChance = 0;
    if (isPet() && GetOwner())
        missChance = uint32(GetOwner()->MeleeSpellMissChance(victim, attType, skillDiff, spell->Id) * 100.0f);
    else
        missChance = uint32(MeleeSpellMissChance(victim, attType, skillDiff, spell->Id) * 100.0f);
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    AuraStateType auraState = aura->GetSpellInfo()->GetAuraState(aura->GetEffectMask());
    if (auraState)
    {
        bool canBreak = false;
...
float Unit::GetTotalAuraMultiplierByMiscMask(AuraType auratype, uint32 misc_mask) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        ...
        {
            bool addPct = true;
            // Maybe this can be generalized to all spells that have
            if (auratype == SPELL_AURA_MOD_CRIT_DAMAGE_BONUS)
            {
                if (ToPlayer() && (*i)->GetSpellInfo()->EquippedItemSubClassMask)
                {
            ...
            if (addPct && !sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup))
                AddPctN(multiplier, (*i)->GetAmount());
        }
    }
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->SpellFamilyFlags[0] & 0x8000000)
            {
                ...
                if (victim)
                    holy += SpellBaseDamageBonusForVictim(SPELL_SCHOOL_MASK_HOLY, victim);
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                {
                    Player* player = ToPlayer();
                    if (!damage || !player || !castItem || !castItem->IsEquipped() || !victim || !victim->isAlive())
                        return false;
            ...
            {
                if (!damage || GetTypeId() != TYPEID_PLAYER  || !victim || !victim->isAlive() || !castItem || !castItem->IsEquipped())
                    return false;
...
uint32 Unit::SpellDamageBonusNOT_USED(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && ToCreature()->isTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellDamageBonusNOT_USED(victim, spellProto, pdamage, damagetype);
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, RollingDot *rdot)
{
    if (!spellProto || !victim || damagetype == DIRECT_DAMAGE)
        return pdamage;
    ...
    /* CUMUL: fait */
    // rollingdots : check saved value from +%damage buffs
    DoneTotalMod += RollingDot::getValue(ROLLING_PCT_DAMAGE, this, victim, spellProto, rdot);
...
//	 test
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            ...
            if (bonus->ap_dot_bonus > 0)
            {
                // rolling dots: apply saved AP here
                float APbonus = RollingDot::getValue(ROLLING_ATTACK_POWER, this, victim, spellProto, rdot);
                DoneTotal += int32(bonus->ap_dot_bonus * stack * ApCoeffMod * APbonus);
            }
        }
        else
...
bool Unit::isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType, RollingDot *rdot) const
{
    float crit_chance = RollingDot::getValue(ROLLING_CRIT_CHANCE, this, victim, spellProto, rdot);
...
float Unit::CalcCritChance(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    switch (spellProto->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_NONE:
        {
            // Spells with SPELL_EFFECT_HEAL should be able to crit, examples: Lifebloom, Earth shield, Frenzied regeneration, Divine Hmyn, Bauble of true blood
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            // For other schools
                ...
                crit_chance = GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1 + GetFirstSchoolInMask(schoolMask));
            else if (isTotem()) //Chance to crit for all active totems and Fire Nova
            {
                if (Unit* TotemsOwner = this->GetOwner()) // maybe ->ToCreature()?
                    crit_chance = TotemsOwner->GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1 + GetFirstSchoolInMask(schoolMask));
            }
            else
            {
            ...
            if (victim)
            {
                if (!spellProto->IsPositive())
                {
                    ...
                    if (victim->HasAura(53480)) // savage roar crit chance = -100
                    {
                        crit_chance -= 100.0f;
                    }
...
uint32 Unit::SpellCriticalDamageBonus(SpellInfo const* spellProto, uint32 damage, Unit* victim, RollingDot *rdot)
{
    ...
    // rollingdots : apply saved critamount value
    crit_mod += RollingDot::getValue(ROLLING_CRIT_AMOUNT, this, victim, spellProto, rdot);
    // (GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_CRIT_DAMAGE_BONUS, spellProto->GetSchoolMask()) - 1.0f) * 100;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorbed, SpellInfo const* procAura)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        if (isVictim && damage && !(procExtra & PROC_EX_CRITICAL_HIT) && !(procFlag & PROC_FLAG_TAKEN_PERIODIC)
            && !(spellProto && spellProto->AttributesEx2 & SPELL_ATTR2_CANT_CRIT))
        {
            if (urand(0, 100) < uint32(GetMeleeCritChanceReduction()))
                tempProcEx |= PROC_EX_CRITICAL_HIT;
        }

        if (!IsTriggeredAtSpellProcEvent(target, triggerData.aura, procSpell, procFlag, tempProcEx, attType, isVictim, active, triggerData.spellProcEvent))
            continue;
        ...
        bool triggered = (isVictim || (spellProto->AttributesCu & SPELL_ATTR0_CU_NEVER_PROC_TRIGGERED)) && !(spellProto->AttributesEx3 & SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED) ?
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    if ((isVictim || (spellProto->AttributesCu & SPELL_ATTR0_CU_NEVER_PROC_TRIGGERED)) && procExtra & PROC_EX_INTERNAL_TRIGGERED && !(procFlag & PROC_FLAG_DONE_TRAP_ACTIVATION))
    {
    ...
    // FIX MAGIE NOIRE    if (spellProto->Id ==
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (spellProto->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            Item* item = NULL;
            Item* tmp = NULL;
            if (tmp = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND))
            {
                if (attType == BASE_ATTACK)
                    item = tmp;
                ItemSubClass |= (1 << tmp->GetTemplate()->SubClass);
            }
            if (tmp = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
            {
                if (attType == OFF_ATTACK)
                    item = tmp;
                ItemSubClass |= (1 << tmp->GetTemplate()->SubClass);
            }
            if (tmp = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED))
            {
                if (attType == RANGED_ATTACK)
                    item = tmp;
                ItemSubClass |= (1 << tmp->GetTemplate()->SubClass);
            ...
            if (!item || item->IsBroken() || item->GetTemplate()->Class != ITEM_CLASS_WEAPON || !(ItemSubClass & spellProto->EquippedItemSubClassMask))
                return false;
        }
...
void Unit::SetFacingTo(float ori)
{
    Movement::MoveSplineInit init(*this);
    init.MoveTo(GetPositionX(), GetPositionY(), GetPositionZMinusOffset());
    if (HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && GetTransport())
