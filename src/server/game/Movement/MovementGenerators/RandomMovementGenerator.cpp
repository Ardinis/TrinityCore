...
void RandomMovementGenerator<Creature>::_setRandomLocation(Creature &unit)
{
    ...
    else                                                    // 2D only
    {
        ...
        destZ = map->GetHeight(unit.GetPhaseMask(), destX, destY, respZ+travelDistZ-2.0f, false);

        if (fabs(destZ - respZ) > travelDistZ)              // Map check
        {
            // Vmap Horizontal or above
            destZ = map->GetHeight(unit.GetPhaseMask(), destX, destY, respZ - 2.0f, true);

            if (fabs(destZ - respZ) > travelDistZ)
            {
                // Vmap Higher
                destZ = map->GetHeight(unit.GetPhaseMask(), destX, destY, respZ+travelDistZ-2.0f, true);
