...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    if (!i_path)
    {
        ...
        sLog->outError( "WaypointMovementGenerator::LoadPath: creature %s (Entry: %u GUID: %u) doesn't have waypoint path id: %u", creature->GetName(), creature->GetEntry(), creature->GetGUIDLow(), path_id);
        return;
    }
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
    ...
    if (endMap)
    {
        sLog->outString("Preloading rid (%f, %f) for map %u at node index %u/%u", _endGridX, _endGridY, _endMapId, _preloadTargetNode, (uint32)(i_path->size()-1));
        endMap->LoadGrid(_endGridX, _endGridY);
    }
    else
        sLog->outString("Unable to determine map to preload flightmaster grid");
}
