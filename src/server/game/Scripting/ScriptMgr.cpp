    ...
    for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
    {
        ...
        for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            if (pTempSpell->Effects[j].TargetA.GetTarget() == TARGET_UNIT_SRC_AREA_ENEMY ||
                ...
                pTempSpell->Effects[j].TargetA.GetTarget() == TARGET_CORPSE_SRC_AREA_ENEMY ||
...
void ScriptMgr::OnGameEventStart(uint16 event_id)
{
    FOREACH_SCRIPT(GameEventScript)->OnEventStart(event_id);
}

void ScriptMgr::OnGameEventStop(uint16 event_id)
{
    FOREACH_SCRIPT(GameEventScript)->OnEventStop(event_id);
}
