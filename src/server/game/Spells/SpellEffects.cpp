        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 55004:                                 // Nitro Boosts
                    if (!m_CastItem)
                        return;
                    if (roll_chance_i(99) || m_caster->GetMap()->IsDungeon())                  // Nitro Boosts - success
        ...
        case 57840:
        {
            if (Unit *spellTarget = m_targets.GetUnitTarget())
                if (Unit *target = unitTarget)
                {
                    ...
                    fang = MapManager::NormalizeOrientation(fang);
...
void Spell::EffectGameObjectDamage(SpellEffIndex /*effIndex*/)
{
    ...
    FactionTemplateEntry const* casterFaction = caster->getFactionTemplateEntry(true);
