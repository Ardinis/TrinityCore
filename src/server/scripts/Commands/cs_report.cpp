...
public:
	...
	if (uint32 quest = atoi(qId))
	  {
	    //  uint32 quest = extractQuestIdFromLink((char*)args);
	    if (!quest)
	      return false;
	    QueryResult result = WorldDatabase.PQuery("SELECT status, count FROM report_quest WHERE id='%d'", quest);
	    if (!result)
	      {
		WorldDatabase.PExecute("INSERT INTO report_quest(id, count, comment) VALUES('%d', '%d', '%s')", quest, count, "");
	    ...
	    if (!resultit)
	      ...
	      {
		//		handler->PSendSysMessage(LANG_REPORT_QUEST_ALREADY_REPORT);
	...
	if (uint32 quest = atoi(qId))
	  {
		...
		//	handler->PSendSysMessage(LANG_REPORT_QUEST_WAITING);
	    ...
	    //  handler->PSendSysMessage(LANG_REPORT_QUEST_SUCCESS);
	    return true;
	  }
      return false;
    }
