...
public:
 ...
 ChatCommand* GetCommands() const
 {
   static ChatCommand spectateCommandTable[] =
     {
       { "spectate", SEC_PLAYER, true, &HandleSpectateCommand, "", NULL },
...
class npc_arena_spectator : public CreatureScript
{
 ...
 bool OnGossipHello(Player* pPlayer, Creature* pCreature)
 {
  ...
  bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
  {
    ...
    switch (action)
    {
    case NPC_SPECTATOR_ACTION_LIST_GAMES:
      ShowPage(player, 1, 4);
      player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
      break;
    case NPC_SPECTATOR_ACTION_LIST_GAMES+1:
      ShowPage(player, 1, 6);
      player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
      break;
    case NPC_SPECTATOR_ACTION_LIST_GAMES+2:
      ShowPage(player, 1, 10);
      player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
      break;
    case 0:
      ShowPage(player, 0, 0);
 ...
 std::string GetGamesStringData(Battleground *arena, uint16 mmr)
 {
  ...
  void ShowPage(Player *player, uint16 page, uint32 teamSize)
  {
    uint16 highGames = 0;
    uint16 lowGames = 0;
