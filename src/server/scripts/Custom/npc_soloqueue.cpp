...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (!sWorld->getBoolConfig(CONFIG_ARENA_SOLO_QUEUE_ENABLED))
        {
            ...
            player->SEND_GOSSIP_MENU(TEXT_SOLO_QUEUE, creature->GetGUID());
            return true;
        }
        ...
        if (cache3v3Queue[4] > 0)
        {
            if ((cache3v3Queue[4] / 1000) < 60)
                infoQueue << "Average wait time: " << cache3v3Queue[4] / 1000 << " secs. \n";
            else
                infoQueue << "Average wait time: " << ceil(cache3v3Queue[4] / 1000 / 60) << " min(s). \n";
        }
        else
            infoQueue <<  "Average wait time: not available. \n";
        ...
        if (sWorld->getBoolConfig(CONFIG_ARENA_SOLO_QUEUE_ALLOW_MMH_AND_RRH))
            infoQueue << "Queued DDs: " << cache3v3Queue[TALENT_CAT_MELEE] + cache3v3Queue[TALENT_CAT_RANGE] << "\n";
        else
        {
        ...
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, infoQueue.str(), GOSSIP_SENDER_MAIN, 0);
            player->SEND_GOSSIP_MENU(TEXT_SOLO_QUEUE, creature->GetGUID());
            return true;
        }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case 1: // Create new solo queue team
            {
                ...
                if (!arenaTeam->Create(player->GetGUID(), ARENA_TEAM_5v5, teamName.str(), 0xFF4588D2, 58, 0xFF47FFE2, 4, 0xFF94EEFF, true))
                {
                    delete arenaTeam;
                    player->CLOSE_GOSSIP_MENU();
                    return true;
