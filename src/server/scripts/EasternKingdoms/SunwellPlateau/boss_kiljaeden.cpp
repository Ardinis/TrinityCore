...
static Speech speeches[]=
{
    ...
    {SAY_ANVEENA_GOODBYE,       DATA_ANVEENA,       8000},
    {SAY_KJ_PHASE5,             DATA_KILJAEDEN,     5500},
    {0,                         0,                  3000},
    {SAY_KALECGOS_ENCOURAGE,    DATA_KALECGOS_KJ,   18000},
                ...
                if (!orbs.empty())
                {
                    ...
                    if (GameObject* orb = GetOrb(*itr))
                    {
                        ...
                        if (firstOrbEmpowered)
                            DoScriptText(RAND(SAY_KALEC_ORB_READY2, SAY_KALEC_ORB_READY3), me);
                        else
                        {
                            ...
                            if (speeches[speechCount].timer < speechTimer)
                            {
                                ...
                                if (instance)
                                    if (Creature* speechCreature = Unit::GetCreature(*me, instance->GetData64(speeches[speechCount].creatureData)))
                                        DoScriptText(speeches[speechCount].textid, speechCreature);
                                if (speechCount == 11)
                                {
                                    if (instance)
...
public:
    ...
    class spell_kiljaeden_sinister_reflection_SpellScript : public SpellScript
    {
        ...
        void Register()
        {
            OnUnitTargetSelect += SpellUnitTargetFn(spell_kiljaeden_sinister_reflection_SpellScript::FilterTargets, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
...
public:
    ...
    class spell_kiljaeden_armageddon_SpellScript : public SpellScript
    {
        ...
        void Register()
        {
            OnEffectLaunch += SpellEffectFn(spell_kiljaeden_armageddon_SpellScript::ModDestHeight, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    class spell_kiljaeden_haste_revit_SpellScript : public SpellScript
    {
        ...
        void Register()
        {
            OnUnitTargetSelect += SpellUnitTargetFn(spell_kiljaeden_haste_revit_SpellScript::FilterTargets, EFFECT_ALL, TARGET_UNIT_CONE_ALLY);
        }
    };
