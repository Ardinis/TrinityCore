                    ...
                    case 2:
                        switch (uiData)
                        {
                            case FAIL:
                                ...
                                {
                                    _instance->SetData(TYPE_CRUSADERS, FAIL);
                                    _summons.DespawnAll();
                            ...
                            case IN_PROGRESS:
                                if (!_inProgress)
                                {
                                    ...
                                    _instance->SetData(TYPE_CRUSADERS, IN_PROGRESS);
                                }
                                break;
                            case DONE:
                                _championsKilled++;
                                if (_championsKilled == 1)
                                    _instance->SetData(TYPE_CRUSADERS, SPECIAL);
                                else if (_championsKilled >= _summons.size())
                                {
                                    _instance->SetData(TYPE_CRUSADERS, DONE);
                                    _summons.DespawnAll();
                                    me->DespawnOrUnsummon();
                                }
                                break;
        ...
        struct mob_toc_warriorAI : public boss_faction_championsAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                        ...
                        case EVENT_SHATTERING_THROW:
			  {
			    ...
			    if (target && target->HasAuraWithMechanic(1 << MECHANIC_IMMUNE_SHIELD))
                            {
