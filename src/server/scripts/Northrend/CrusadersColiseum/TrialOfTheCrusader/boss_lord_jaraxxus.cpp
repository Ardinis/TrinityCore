    ...
    struct mob_infernal_volcanoAI : public Scripted_NoMovementAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (m_Timer <= uiDiff)
            {
                ...
                {
		  me->SummonCreature(34815, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), TEMPSUMMON_CORPSE_DESPAWN);
...
public:
    ...
    struct mob_fel_infernalAI : public ScriptedAI
    {
        ...
        /*void SpellHitTarget(Unit *pTarget, SpellInfo const*pSpell)
        {
