...
struct boss_twin_baseAI : public ScriptedAI
{
  ...
  void RemoveBadAuras()
  {
    ...
    {
      //if (urand(0, 1))
      //return Unit::GetCreature((*me), m_pInstance->GetData64(NPC_LIGHTBANE));
      //else
      //return Unit::GetCreature((*me), m_pInstance->GetData64(NPC_DARKBANE));
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
		...
		player->RemoveAura(67176);
		player->RemoveAura(67177);
		player->RemoveAura(67178);
                player->CastSpell(player, SPELL_LIGHT_ESSENCE, true);
                break;
            case NPC_DARK_ESSENCE:
                player->RemoveAura(SPELL_LIGHT_ESSENCE);
		player->RemoveAura(67222);
		player->RemoveAura(67223);
		player->RemoveAura(67224);
