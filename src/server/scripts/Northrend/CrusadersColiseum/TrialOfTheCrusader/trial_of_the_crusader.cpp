...
class npc_announcer_toc10 : public CreatureScript
{
    public:
	    ...
	    instanceScript->SetData(DATA_PAUSE, IN_PROGRESS);
                ...
                if (m_uiUpdateTimer <= uiDiff)
                {
                    switch (m_pInstance->GetData(TYPE_EVENT))
                    {
                        ...
                        case 310:
                            if (m_pInstance->GetData(TYPE_BEASTS) != DONE)
                            {
			      me->SummonCreature(NPC_ICEHOWL, ToCCommonLoc[10].GetPositionX(), ToCCommonLoc[10].GetPositionY(), ToCCommonLoc[10].GetPositionZ(), 5, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 240*IN_MILLISECONDS);
