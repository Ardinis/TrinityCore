    ...
    public:
        ...
        struct boss_algalonAI : public BossAI
        {
					...
					if (!players.isEmpty())
					{
						for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
						{
							if (Player* plr = itr->getSource())
							{
								...
								if (Item *oh=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_MAINHAND))
									if(oh->GetTemplate()->GetItemLevelIncludingQuality()>232)
										return false;
								if (Item *mh=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_OFFHAND))
									if(mh->GetTemplate()->GetItemLevelIncludingQuality()>232)
										return false;
								if (Item *ranged=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_RANGED))
									if(ranged->GetTemplate()->GetItemLevelIncludingQuality()>232)
										return false;
								if (Item *head=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_HEAD))
									if(head->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *neck=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_NECK))
									if(neck->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *shoulders=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_SHOULDERS))
									if(shoulders->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *chest=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_CHEST))
									if(chest->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *waist=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_WAIST))
									if(waist->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *legs=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_LEGS))
									if(legs->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *feet=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_FEET))
									if(feet->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *wrists=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_WRISTS))
									if(wrists->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *hands=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_HANDS))
									if(hands->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *finger=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_FINGER1))
									if(finger->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *finger=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_FINGER2))
									if(finger->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *trinket=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_TRINKET1))
									if(trinket->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *trinket=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_TRINKET2))
									if(trinket->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
								if (Item *back=plr->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_BACK))
									if(back->GetTemplate()->GetItemLevelIncludingQuality()>226)
										return false;
							}
						}
						return true;
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                {
					if (HandleHeraldAchievement()==true)
						if(!Is25ManRaid())
							instance->DoCompleteAchievement(3316);
                    Phase = PHASE_INTRO;
                    summons.DespawnAll();
					...
					if(Is25ManRaid())
						me->SummonGameObject(194822, RoomCenter[0], RoomCenter[1], RoomCenter[2], M_PI/2,0,0,0,0,me->GetRespawnTime()); //should be DB spawned and then used via Respawn by the Instance script.
					else
