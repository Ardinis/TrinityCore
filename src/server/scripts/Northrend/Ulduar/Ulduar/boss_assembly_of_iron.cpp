    ...
    public:
        ...
        struct boss_stormcaller_brundirAI : public BossAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
				...
				//on file le HF can't while stunned si toujours pas touche
				if(toucheParEclair==false)
			...
			//gerer HF can't while stunned
			void SetData(uint32 type, uint32 data)
            {
				if(type==42)
					toucheParEclair=true;
            }

        private:
            uint32 _phase;
            bool _forceLand;
			bool toucheParEclair;
        };
...
//debug achievement can't while stunned ulduar
class spell_achievement_stunned_ulduar : public SpellScriptLoader
{
    public:
        spell_achievement_stunned_ulduar() : SpellScriptLoader("spell_achievement_stunned_ulduar") {}

        class spell_achievement_stunned_ulduarSpellScript : public SpellScript
        {
            PrepareSpellScript(spell_achievement_stunned_ulduarSpellScript)

            void HandleBeforeHit()
            {
				if (Creature* Brundir = GetCaster()->ToCreature())
				{
					Brundir->GetAI()->SetData(42,0);
				}
            ...
            void Register()
            {
                BeforeHit += SpellHitFn(spell_achievement_stunned_ulduarSpellScript::HandleBeforeHit);
            }
        };

        SpellScript *GetSpellScript() const
        {
            return new spell_achievement_stunned_ulduarSpellScript();
        }
};
void AddSC_boss_assembly_of_iron()
{
	...
	new spell_achievement_stunned_ulduar();
}
