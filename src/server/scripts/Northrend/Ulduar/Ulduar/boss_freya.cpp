...
enum Spells
{
	...
	//Spell pour donner le HF deforestation
...
public:
    ...
    struct boss_freyaAI : public ScriptedAI
    {
		...
		bool TimerEnRoute;
		bool achDeforestationMerite;
		uint32 timerDeforestation; 
		int nbelem1killed;
		int nbelem2killed;
		int nbelem3killed;
        ...
        void Reset()
        {
            ...
            else
            {
				...
				timerDeforestation=20000;
				nbelem1killed=0;
				nbelem2killed=0;
				nbelem3killed=0;
				TimerEnRoute=false;
				achDeforestationMerite=false;
		...
		*/
		void SetData(uint32 uiType, uint32 data)
		{
			...
			//verification du HF
			if(nbelem1killed>=2 && nbelem2killed>=2 && nbelem3killed>=2)
			{
				achDeforestationMerite=true;
			}
		}
        ...
        void EncounterIsDone()
        {
			...
			//Ajout de Deforestation si réussit
			if(achDeforestationMerite)
			{
					DoCast(me,SPELL_DEFORESTATION_CREDIT,true);
			}
        }
        ...
        void UpdateAI(uint32 const diff)
        {
			...
			/*gérance du timer de deforestation
			Si on a tué un elem, on lance le timer et donc on decompte
			Si les 20 secondes sont passées, on stop le timer et on le reinitialise à 20 secondes jusqu'au prochain kill d'elem
			On reinitialise aussi le nb des elems tués
			*/
			if (timerDeforestation < diff)
            {
				timerDeforestation=20000;
				TimerEnRoute=false;
				nbelem1killed=0;
...
public:
    ...
    struct mob_detonating_lasherAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            if (Unit* target = me->SelectNearbyTarget(NULL, 100))
                AttackStart(target);
...
public:
    ...
    struct mob_elder_brightleafAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if (Creature* Ironbranch = Unit::GetCreature(*me, m_pInstance ? m_pInstance->GetData64(TYPE_ELDER_IRONBRANCH) : 0))
                if (Ironbranch->isAlive())
                    EldersCount++;
                
            if (Creature* Stonebark = Unit::GetCreature(*me, m_pInstance ? m_pInstance->GetData64(TYPE_ELDER_STONEBARK) : 0))
   ...
   mob_elder_ironbranch() : CreatureScript("mob_elder_ironbranch") { }
    ...
    struct mob_elder_ironbranchAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if (Creature* Brightleaf = Unit::GetCreature(*me, m_pInstance ? m_pInstance->GetData64(TYPE_ELDER_BRIGHTLEAF) : 0))
