...
public:
	...
	/* HACK: J'ai pas retrouve le bon spell ID pour les glacons qui tombent a FDS donc j utilise les glacons de Hodir
	 * Si on est a FDS, alors on verifie ici, pour le HF "ne regardez pas en haut" */ 
	void SpellHitTarget(Unit* target, SpellInfo const* /*spell*/) {
	    if (me->GetMapId() != 658 /* FDS */)
		return;
	    if (target->ToPlayer()) {
		sLog->outError("%s a fait foirer le HF", target->GetName());
		me->GetInstanceScript()->SetData(9 /* DATA_HAUT_FAIT de pit_of_saron.h */ , 0);
	    }
	}
