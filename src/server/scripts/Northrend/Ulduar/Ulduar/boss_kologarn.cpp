...
class boss_kologarn : public CreatureScript
{
    public:
        ...
        struct boss_kologarnAI : public BossAI
        {
			...
			//HF ne pas se faire toucher par eyebeam: Si on pouvait tuer d'un simple regard
    ...
    public:
        ...
        class spell_ulduar_stone_grip_AuraScript : public AuraScript
        {
            ...
            void OnRemoveVehicle(AuraEffect const* /*aurEff*/, AuraEffectHandleModes mode)
            {
                ...
                pCaster->ExitVehicle();
                pCaster->GetMotionMaster()->MoveJump(1756.25f + irand(-3, 3), -8.3f + irand(-3, 3), 448.8f, 10.0f, 5.0f);
...
//debug achievement kologarn eyebeam
class spell_achievement_kologarn_eyebeam : public SpellScriptLoader
{
    public:
        spell_achievement_kologarn_eyebeam() : SpellScriptLoader("spell_achievement_kologarn_eyebeam") {}

        class spell_achievement_kologarn_eyebeamSpellScript : public SpellScript
        {
            PrepareSpellScript(spell_achievement_kologarn_eyebeamSpellScript)

            void HandleOnHit()
            {
				if (Creature *kolo=GetCaster()->ToTempSummon()->GetSummoner()->ToCreature())
					kolo->GetAI()->SetData(17,0);
            }

            void Register()
            {
                OnHit += SpellHitFn(spell_achievement_kologarn_eyebeamSpellScript::HandleOnHit);
            }
        };

        SpellScript *GetSpellScript() const
        {
            return new spell_achievement_kologarn_eyebeamSpellScript();
        }
};
...
void AddSC_boss_kologarn()
{
	...
	new spell_achievement_kologarn_eyebeam();
}
