    ...
    struct boss_xt002_AI : public BossAI
    {
        ...
        void SetPhaseOne()
        {
            ...
            if (!hardMode)
            {
                ...
                if (me->GetHealth() <= 0) {
                    me->SetHealth(1);
                    sLog->outError("xt002: hack de merde (1)"); // Virer et remplacer le ModifyHealth par un DealDamage() quand on sera sur que le bug vient d'ici (cf les logs)
                    me->Kill(me);
                }
            }
...
class achievement_nerf_gravity_bombs : public AchievementCriteriaScript
{
    public:
        ...
        bool OnCheck(Player* source, Unit* target)
        {
            ...
            if (boss_xt002::boss_xt002_AI* xt002AI = CAST_AI(boss_xt002::boss_xt002_AI, target->GetAI()))
