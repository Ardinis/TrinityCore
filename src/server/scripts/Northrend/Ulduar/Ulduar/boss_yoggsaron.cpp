...
class BrainLinkTargetSelector : public std::unary_function<WorldObject*, bool>
{
    public:
        bool operator() (WorldObject* target)
        {
            return target->GetPositionZ() < 300.0f;
        }
};
    ...
    public:
		...
		case ACTION_USED_MINDCONTROL:
	      ...
	      instance->SetData(BOSS_YOGGSARON, NOT_STARTED);
            ...
            void ModifySanityPct(Player* target, int8 amount)  // TODO: Check if stack-amount-handling by Freyas Well works correctly
            {
                if (target)
                    if (target->isAlive())
                    {
                    ...
                    case PHASE_SARA:
                        {
                            ...
                            SetSanityAura();
                            uint32 keeperActiveCnt = CountKeepersActive();
                            if (Creature* yogg = ObjectAccessor::GetCreature(*me, instance->GetData64(BOSS_YOGGSARON)))
                            {
                                yogg->SetLootMode(LOOT_MODE_DEFAULT);
    ...
    public:
		...
		Reset();
                        ...
                        switch (spell->Id)
                        {
                            case SPELL_PSYCHOSIS:
                                if (Creature* ctrl = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_YOGGSARON_CTRL)))
                                    CAST_AI(npc_yogg_saron_encounter_controller::npc_yogg_saron_encounter_controllerAI, ctrl->AI())->ModifySanityPct(target->ToPlayer(), -12);
                                break;
                            case SPELL_BRAIN_LINK_DAMAGE:
                                if (Creature* ctrl = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_YOGGSARON_CTRL)))
                                    CAST_AI(npc_yogg_saron_encounter_controller::npc_yogg_saron_encounter_controllerAI, ctrl->AI())->ModifySanityPct(target->ToPlayer(), -2);
                                break;
                            case SPELL_MALADY_OF_MIND:
                                if (Creature* ctrl = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_YOGGSARON_CTRL)))
                                    CAST_AI(npc_yogg_saron_encounter_controller::npc_yogg_saron_encounter_controllerAI, ctrl->AI())->ModifySanityPct(target->ToPlayer(), -3);
                                break;
                        }
                    }
            }
            ...
            void UpdatePhase(BossPhase newPhase)
            {
                ...
                switch (newPhase)
                {
                    case PHASE_SARA:
                        ...
                        events.ScheduleEvent(EVENT_SARAHS_HELP, urand(5000, 6000), PHASE_SARA);
                        events.ScheduleEvent(EVENT_SUMMON_GUARDIAN, 10000, 0, PHASE_SARA);
                        break;
                    case PHASE_BRAIN:
                        if (Creature* ctrl = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_YOGGSARON_CTRL)))
                            ctrl->AI()->DoAction(ACTION_START_PHASE_2);
                        events.ScheduleEvent(EVENT_MINDSPELL, 5000, 0, PHASE_BRAIN);
                        me->setFaction(FACTION_HOSTILE_ALL);
                        me->SetHealth(me->GetMaxHealth());
                        SpeakingPhase = 0;
                        events.CancelEvent(EVENT_EVTSPEAKING);
                        events.RescheduleEvent(EVENT_SPEAKING, 1000, 0, myPhase);
                        IsEventSpeaking = false;
                        EventSpeakingPhase = 0;
                ...
                if (EventNpcSpeaching[speachindex+part].npc_entry != NPC_YOGG_SARON)          // Why differ at this point ?
		  {
                    ...
                    npcguid = instance->GetData64(BOSS_YOGGSARON);
				...
				target->CastSpell(target, SPELL_SUMMON_GUARDIAN, true);
			      events.ScheduleEvent(EVENT_SUMMON_GUARDIAN, 20000, 0, PHASE_SARA);
                        ...
                        case EVENT_PSYCHOSIS:
                            if (Player* target = SelectPlayerTargetInRange(me, 500.0f))
                                if (!IsPlayerInBrainRoom(target))
                                    me->CastCustomSpell(SPELL_PSYCHOSIS, SPELLVALUE_MAX_TARGETS, 1, me);
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                if (instance)
                {
                    if (Creature* cSara = ObjectAccessor::GetCreature(*me,instance->GetData64(BOSS_SARA)))
                        if (me->GetDistance2d(cSara) <= 100.0f)
...
class boss_yogg_saron : public CreatureScript
{
    ...
    public:
		...
		events.ScheduleEvent(EVENT_TENTACLE_1, 1000, ID_GROUP_NON_SHATTERED, PHASE_BRAIN);
                    ...
                    if (target->ToPlayer())
                    {
                        switch(spell->Id)
                        {
                            case SPELL_LUNATIC_GAZE_EFFECT:
                                if(Creature* ctrl = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_YOGGSARON_CTRL)))
                                    CAST_AI(npc_yogg_saron_encounter_controller::npc_yogg_saron_encounter_controllerAI, ctrl->AI())->ModifySanityPct(target->ToPlayer(), -4);
                                break;
                        }
                    }
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                if( (instance && instance->GetData(BOSS_YOGGSARON) != IN_PROGRESS) || myphase == PHASE_NONE )
                    return;
                ...
                while (uint32 event = events.ExecuteEvent())
                {
                    switch (event)
                    {
                        ...
                        case EVENT_LUNATIC_GAZE:
                            ...
                            me->CastSpell(me, SPELL_LUNATIC_GAZE, false);
                ...
                if(sanityTick_Timer <= diff)
                {
                    ...
                    for (std::list<Player*>::const_iterator itr = plrList.begin(); itr != plrList.end(); ++itr)
                    {
                        if ((*itr))
                        {
                            ...
                            {
                                if (Creature* ctrl = ObjectAccessor::GetCreature((*me), instance->GetData64(NPC_YOGGSARON_CTRL)))
                                    CAST_AI(npc_yogg_saron_encounter_controller::npc_yogg_saron_encounter_controllerAI, ctrl->AI())->ModifySanityPct((*itr), 10);
                            }
                        }
                    }
                    sanityTick_Timer = 2000;
...
class spell_lunatic_gaze_targeting : public SpellScriptLoader
{
    public:
        ...
        class spell_lunatic_gaze_targeting_SpellScript : public SpellScript
        {
            ...
            {
                std::cout << "FilterTargets" << std::endl;
                                    ...
                                    if (!player->IsWithinDist(trigger, float(aurEff->GetAmount())))
                                    {
                                        ...
                                        if(InstanceScript* pInstance = caster->GetInstanceScript())
                                            if(caster->ToCreature() && caster->GetGUID() == pInstance->GetData64(NPC_YOGGSARON_CTRL))
                                                CAST_AI(npc_yogg_saron_encounter_controller::npc_yogg_saron_encounter_controllerAI, caster->ToCreature()->AI())->ModifySanityPct(player,-2);
                                    }
                                }
                    }
                }
...
class spell_titanic_storm_targeting : public SpellScriptLoader
{
    public:
        ...
        class spell_titanic_storm_targeting_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_titanic_storm_targeting_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
...
class npc_keeper_help : public CreatureScript
{
    public:
        ...
        struct npc_keeper_helpAI : public Scripted_NoMovementAI
        {
            ...
            {
                if(instance)
                    me->SetVisible( (instance->GetBossState(Entry_2_ID(me->GetEntry())) == DONE) && (instance->GetData(BOSS_YOGGSARON) != IN_PROGRESS) );
            }
