    ...
    struct npc_guardian_of_yogg_saronAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if(m_pInstance)
            {
				...
				if (m_pInstance->GetData(DATA_COMING_OUT_START) == ACHI_IS_NOT_STARTED)
                    m_pInstance->SetData(DATA_COMING_OUT_START, ACHI_START);

                m_pInstance->SetData(DATA_COMING_OUT_COUNT, ACHI_INCREASE);
            }
    ...
    struct boss_yogg_saronAI : public Scripted_NoMovementAI
    {
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell)
        {
            if (spell->Id == SPELL_IN_THE_MAWS_OF_THE_OLD_GOD)
                if(Creature* cSara = me->GetCreature(*me,m_pInstance->GetData64(TYPE_SARA)))
                    if (CAST_AI(boss_sara::boss_saraAI,cSara->AI())->uiAmountKeeperActive <= 3)
                        if (Is25ManRaid())
    ...
    struct npc_death_orbAI : public Scripted_NoMovementAI
    {
        ...
        void SummonDeathRays()
        {
            for(uint8 i = 0; i < 4; i++)
            {
                ...
                pos.m_positionZ = me->GetMap()->GetHeight(me->GetPhaseMask(), pos.GetPositionX(),pos.GetPositionY(),pos.GetPositionZ(),true,500.0f);
...
public:
    ...
    struct npc_death_rayAI : public ScriptedAI
    {
        ...
        void DoRandomMove()
        {
            ...
            pos.m_positionZ = me->GetPositionZ();
            pos.m_positionZ = me->GetMap()->GetHeight(me->GetPhaseMask(), pos.GetPositionX(),pos.GetPositionY(),pos.GetPositionZ(),true,50.0f);
    ...
    public:
        ...
        class spell_summon_tentacle_position_SpellScript : public SpellScript
        {
            ...
            void ChangeSummonPos(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* caster = GetCaster())
                    summonPos.m_positionZ = caster->GetMap()->GetHeight(caster->GetPhaseMask(), summonPos.GetPositionX(), summonPos.GetPositionY(), summonPos.GetPositionZ());
