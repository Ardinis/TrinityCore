...
bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    ...
    if(pPlayer->isGameMaster() || pCreature->GetZoneScript() && pCreature->GetZoneScript()->GetData(pCreature->GetDBTableGUIDLow()))
    {
      Creature* spawnpospnj;
      if (!(spawnpospnj = pCreature->FindNearestCreature(23472, 50.0f)))
	return true;
	...
	return true;
      ...
      spawnpospnj->GetPosition(destX, destY, destZ);
	  ...
	  /*            case 0: pPlayer->CastSpell(pPlayer, 56663, false, NULL, NULL, pCreature->GetGUID()); break;
	      ...
	      if (Creature *sum = pCreature->SummonCreature(27881 ,destX, destY, destZ))
		{
	      ...
	      break;
            case 1: 
	      if (Creature *sum = pCreature->SummonCreature(28094 ,destX, destY, destZ))
		{
		  sum->setFaction(pCreature->getFaction());
		  pvpWG->OnCreatureCreate(sum);
		  pvpWG->playersVehicles[pPlayer] = 120000;
	      ...
	      break;
            case 2: 
	      if (Creature *sum = pCreature->SummonCreature(pPlayer->GetTeamId() ? 32627 : 28312,destX, destY, destZ))
		{
		  sum->setFaction(pCreature->getFaction());
		  pvpWG->OnCreatureCreate(sum);
		  pvpWG->playersVehicles[pPlayer] = 120000;
...
class spell_summon_vehicle_wintergrasp : public SpellScriptLoader
{
public:
  spell_summon_vehicle_wintergrasp() : SpellScriptLoader("spell_summon_vehicle_wintergrasp") { }

  class spell_summon_vehicle_wintergrasp_SpellScript : public SpellScript
  {
    PrepareSpellScript(spell_summon_vehicle_wintergrasp_SpellScript);
    ...
    void Register()
    {
      OnHit += SpellHitFn(spell_summon_vehicle_wintergrasp_SpellScript::d);
    }
  };

  SpellScript* GetSpellScript() const
  {
    return new spell_summon_vehicle_wintergrasp_SpellScript();
  }
};
