...
#include "CataCharacterDatabase.h"

void CataCharacterDatabaseConnection::DoPrepareStatements()
{
    if (!m_reconnecting)
        m_stmts.resize(MAX_CATACHARACTERDATABASE_STATEMENTS);

    // Player saving
    PREPARE_STATEMENT(CHAR_INS_CATACHARACTER, "INSERT INTO characters (guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
    ...
    "latency, speccount, activespec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels, achievementPoint) VALUES "
    "(?,?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    ...
    "health, power1, power2, power3, power4, power5, power6, power7, instance_id, speccount, activespec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, XPRate FROM characters WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_INS_MAIL_ITEM, "REPLACE INTO mail_items(mail_id, item_guid, receiver) VALUES (?, ?, ?)", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_INS_PVESTATS_GUILD, "INSERT INTO pvestats_guild (guild_id, boss_entry, kill_timer, death_count, difficulty, date) VALUES (?, ?, ?, ?, ?, NOW())", CONNECTION_ASYNC);
    ...
    "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,achievementPoint=?, online=?, XPRate=? WHERE guid=?", CONNECTION_ASYNC);
